...
function parseRequestBody(
  ...
  return { required: required ?? false, schemaRef: schema.$ref };
}
...
function parseResponses(
  ...
  for (const [statusCodeStr, response] of Object.entries(responses ?? {})) {
    ...
    if (isNaN(statusCode)) {
      throw new Error(
        `Invalid status code '${statusCodeStr}' in responses of operation '${operationId}'.`,
      );
    }
    ...
    if (!content) {
      parsed.push({ statusCode, description });
      continue;
    }
    ...
    parsed.push({ statusCode, description, schemaRef: schema.$ref });
  }
...
function parseOperation(
  ...
  if (operation.requestBody) {
    if (isReferenceObject(operation.requestBody)) {
      throw new Error(
        `Request body $ref is not supported in operation '${operationId}' at path '${path}'.`,
      );
    }

    requestBody = parseRequestBody(operation.requestBody, operationId);
  }

  const responses = parseResponses(operation.responses, operationId);
