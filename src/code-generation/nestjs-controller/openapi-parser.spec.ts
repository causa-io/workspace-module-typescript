...
`;
    ...
    expect(createOp!.requestBodyRef).toEqual('./dtos/post-create.dto.yaml');
...
`;
  ...
  it('should parse only the success response', async () => {
    const spec = `
openapi: 3.1.0
x-causaResourceName: Post
info:
...
`;
    ...
    expect(deleteOp!.successResponse).toEqual({
      statusCode: 204,
      description: 'Deleted successfully',
    });
  });
