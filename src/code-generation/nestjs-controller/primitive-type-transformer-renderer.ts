...
/**
 * A {@link TypeScriptWithDecoratorsRenderer} that adds `@Type(() => ...)` decorators from `class-transformer`
 * to convert non-string primitive types (numbers, booleans) from their string representation.
 *
 * This is useful for path and query parameters in NestJS controllers, which are received as strings
 * and need to be converted to their proper JavaScript types.
 */
